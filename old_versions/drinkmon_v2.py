...
VERBOSE_MODE = False  # Set to False to disable verbose output
"""
...
# App logic params
END_TIMEOUT      = (60 * 1)   # seconds
THRESH_MM        = 100  # 10 cm
SENSOR_PERIOD    = 1.0  # seconds
BREATH_PERIOD_MS = 2000 # 1s per breath
POLL_INTERVAL    = 30   # friend session poll interval (seconds)

FRIEND_POLL_URL = 'https://drinkmon.chrispatten.dev/api/friend_sessions'
START_POST_URL  = 'https://drinkmon.chrispatten.dev/api/start_session'
END_POST_URL    = 'https://drinkmon.chrispatten.dev/api/end_session'
...
async def button_monitor_task() -> None:
    ...
    debounce_ms = 200
    last_press = 0
    global user_active, start_ts, session_guid
    while True:
        if button and button.value() == 0:  # Button pressed (active low)
            ...
            if now - last_press > debounce_ms:
                if user_active:
                    ...
                    # Deactivate session via API if GUID is available
                    if session_guid and urequests:
                        try:
                            payload = {"guid": session_guid}
                            resp = urequests.post("https://drinkmon.chrispatten.dev/api/close_session", json=payload)
                            if resp.status_code == 200:
                                print("Session closed on server.")
                            else:
                                print(f"Session close POST failed: {resp.status_code}")
                            resp.close()
...
friend_colors = []
start_ts = 0
session_guid = None  # GUID for the current session (None if inactive)
...
def friend_poll_function() -> list:
    """
    Poll the friend session API and return list of colors.
...
async def friend_poll_task():
    ...
    while True:
        friend_colors = friend_poll_function()
        await asyncio.sleep(POLL_INTERVAL)

async def sensor_task(MY_COLOR):
    """
    Monitor the VL53L0X sensor and manage session state and LED color.
    - Keeps LED off until a session starts.
    - Only starts a session after successful POST to START_POST_URL.
    - Updates LED color only when user_active is True.
    - Ends session after END_TIMEOUT seconds of inactivity.

    Parameters:
        MY_COLOR (tuple): RGB color tuple for this device/user.
    """
    global user_active, start_ts, friend_colors, session_guid
    while True:
        ...
        now = utime.time()
        # Only show session color if user_active
        if user_active and LASER_DEBUG_MODE:
            if d is not None and d > 0:
                rgb = color_for_distance(d)
                set_color(rgb, 1.0)
            else:
            ...
            # LED off when not in session
            set_color((0,0,0), 0)
        # Session logic
        # If distance exceeds threshold and no session is active, start a new session by POSTing device color to API.
        if d is not None and d > THRESH_MM:
            if not user_active:
                # Try to start session by POSTing to server with correct payload
                if urequests:
                    try:
                        payload = {"color": {"r": MY_COLOR[0], "g": MY_COLOR[1], "b": MY_COLOR[2]}}
                        resp = urequests.post(START_POST_URL, json=payload)
                        if resp.status_code == 200:
                            # Capture GUID from response
                            resp_json = resp.json()
                            session_guid = resp_json.get("guid")
                            user_active = True
                            start_ts = now
                            print(f"Session started! GUID: {session_guid}")
                        else:
                            print(f"Session start POST failed: {resp.status_code}")
                        resp.close()
                    except Exception as e:
